...
class Machine:
    ...
    1. downwards are functions that perform the simulation
       including production, idle and breakdown
    '''
    ...
    def initialization(self, machine_list, workcenter_list, event_creator, assigned_wc):
    ...
    def production(self):
        ...
        while True:
            ...
            if not self.working_event.triggered:
                ...
                self.state_update_all()
            # check the queue/stock level, if none, idle begines
            if not len(self.queue):
                # triggered the idle
                yield self.env.process(self.idle())
    ...
    def idle(self):
        self.logger.info("IDL: Machine {} becomes idle at time {}".format(self.m_idx, self.env.now))
        ...
        if not self.working_event.triggered:
            yield self.env.process(self.breakdown())
        self.logger.info("IDL: Machine {} started at time {}".format(self.m_idx, self.env.now))
    ...
    def breakdown(self):
        ...
        self.logger("BKD: Machine {} restored at time {}, down for {} units".format(self.m_idx, self.env.now, self.env.now - start))
    ...
    def after_operation(self):
        ...
        # if this is the last process, then simply delete job information
            ...
            self.event_creator.record_job_departure()
            #print(self.event_creator.in_system_job_no)
            # calculate slack gain/loss
            self.slack_change = current_slack - estimated_slack_time
            self.critical_level_R = 1 - estimated_slack_time / 100
    ...
    def record_production(self, pt, wait):
        # add the details of operation to event_creator's repository
        self.event_creator.production_record[self.job_idx][0].append((self.env.now,pt))
        self.event_creator.production_record[self.job_idx][1].append(self.m_idx)
        self.event_creator.production_record[self.job_idx][2].append(wait)

    def record_slack_tardiness(self, *args):
        self.event_creator.production_record[self.job_idx][4].append(self.slack_change)
        if len(args):
            self.event_creator.production_record[self.job_idx].append((self.env.now,args[0]))
    ...
    def add_global_reward_RA(self): 
        job_record = self.event_creator.production_record[self.job_idx]
        path = job_record[1]
