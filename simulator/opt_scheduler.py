...
class OPT_scheduler:
    def __init__(self, *args, **kwargs):
        for k, v in kwargs.items():
            setattr(self, k, v)
        self.schedule = {m:[] for m in self.m_list}


    def solve_problem(self):
        # get the snapshot of system
        self.jobs_dict = self.recorder.in_system_jobs
        self.machines_list = self.m_list
        # check if optimization is needed
        if len(self.jobs_dict) == 1:
            self.solve_without_optimization()
        # if more than one job but no intersection
        elif len(self.jobs_dict) == 2:
            self.solve_without_optimization()
        else:
            self.solve_without_optimization()
    ...
    def solve_without_optimization(self):
        for _j_idx, _j_object in self.jobs_dict.items():
            _j_object.remaining_operations
    ...
    def draw_from_schedule(self, _jobs:list, _m_idx:int, *args) -> int:
        pass