...
class Brain:
    def __init__(self, env, event_creator, machines, warm_up, span, *args, **kwargs):
        ...
        self.event_creator.build_sqc_experience_repository(self.m_list)
    ...
    def state_direct(self, sqc_data): # presenting information of job which satisfies certain criteria
        ...
        arriving_jobs = np.where(self.event_creator.next_machine_list == sqc_data[-1])[0] # see if there are jobs will arrive
        ...
        avlm_self = self.event_creator.available_time_list[sqc_data[-1]] - self.env.now
        #print(self.event_creator.next_machine_list, self.event_creator.release_time_list, self.env.now)
        ...
        if self.arriving_job_exists: # if there are arriving jobs
            ...
            pos = arriving_jobs[self.event_creator.release_time_list[arriving_jobs].argmin()]
            arriving_j_idx = self.event_creator.current_j_idx_list[pos]
            # and retrive the information of this job
            pt_self = self.event_creator.next_pt_list[pos]
            rem_pt = self.event_creator.arriving_job_rempt_list[pos]
            slack = self.event_creator.arriving_job_slack_list[pos]
            self.strategic_idleness_time = self.event_creator.release_time_list[arriving_jobs].min() - self.env.now # how long to wait if agent decide to wait for arriving job
    ...
    def build_experience(self,j_idx,m_idx,s_t,a_t):
        self.event_creator.incomplete_rep_memo[m_idx][self.env.now] = [s_t,a_t]
    ...
    def build_initial_rep_memo_parameter_sharing(self):
        #print(self.event_creator.rep_memo)
        for m in self.m_list:
            # copy the initial memoery from corresponding rep_memo from job creator
            #print('%s complete and %s incomplete experience for machine %s'%(len(self.event_creator.rep_memo[m.m_idx]), len(self.event_creator.incomplete_rep_memo[m.m_idx]), m.m_idx))
            #print(self.event_creator.incomplete_rep_memo[m.m_idx])
            self.rep_memo += self.event_creator.rep_memo[m.m_idx].copy()
            # and clear the replay memory in job creator, keep it updated
            self.event_creator.rep_memo[m.m_idx] = []
    ...
    def update_rep_memo_parameter_sharing_process(self):
        ...
        while self.env.now < self.span:
            for m in self.m_list:
                ...
                self.rep_memo_TDerror = torch.cat([self.rep_memo_TDerror, torch.ones(len(self.event_creator.rep_memo[m.m_idx]),dtype=torch.float)])
                # and clear the replay memory in job creator, keep it updated
                self.event_creator.rep_memo[m.m_idx] = []
    ...
    def check_parameter(self):
        ...
        print("PT heterogeneity:",self.event_creator.pt_range)
        print('Due date tightness:',self.event_creator.tightness)
        print('Utilization rate:',self.event_creator.E_utliz)
        print('----------------------------------------------------')
